...
{
    public partial class AuditoriaLubricantes : System.Web.UI.Page
    {
        ...
        public static string PuntoxResp(string resp)
        {
            ...
            if (resp == "TODOS")
            {
                vpuntos = new XPQuery<v_Punto>(session).Where(x => x.Subtema != null && x.Lubricantes == true).ToList();
            }
            else
            {
                vpuntos = new XPQuery<v_Punto>(session).Where(x => x.FiguraResponsable.ToString() == resp && x.Subtema != null && x.Lubricantes == true).ToList();
            }
